...
Using the function we just defined, let's take a look at the normalized distribution of store-level weekly sales. We hope this is roughly normal as it will allow us to apply some statistical tests with more confidence in the coming analysis.

```{r}
require(ggplot2)
weekly.sales.by.store <- get.aggr.sales(master, c('Date', 'Store'))
ggplot(data=weekly.sales.by.store, aes(x=Norm_Sales)) + geom_histogram(binwidth=0.5)
```
...
```{r boxplot1}
sales.by.unemployment <- get.aggr.sales(master, c('Store', 'Date', 'Unemployment'))
...
sales.by.norm.unemployment <- get.aggr.sales(master, c('Store', 'Date', 'Norm_Unemployment'))
...
We repeat the same procedure for CPI and fuel price. The p-values for the various tests performed for each economic factor are summarized in the table below:
...
# CPI
master <- add.normalized.col(master, 'CPI')
sales.by.norm.cpi <- get.aggr.sales(master, c('Store', 'Date', 'Norm_CPI'))
ggplot(data=sales.by.norm.cpi, aes(x=Norm_CPI, y=Norm_Sales, 
  group=round_any(Norm_CPI, 0.25))) + geom_boxplot()
sales.by.norm.cpi$CPI_Category <- ifelse(
  sales.by.norm.cpi$Norm_CPI < 0, 'Below Average', 'Above Average')
ggplot(data=sales.by.norm.cpi, aes(x=Norm_Sales)) + geom_histogram(binwidth=4) +
  facet_wrap(~CPI_Category)
above <- subset(sales.by.norm.cpi, CPI_Category == 'Above Average')
below <- subset(sales.by.norm.cpi, CPI_Category == 'Below Average')
var.test(above$Norm_Sales, below$Norm_Sales)
t.test(above$Norm_Sales, below$Norm_Sales, var.equal=FALSE)
wilcox.test(Norm_Sales ~ CPI_Category, data=sales.by.norm.cpi)

# fuel price
master <- add.normalized.col(master, 'Fuel_Price')
sales.by.norm.fuel <- get.aggr.sales(master, c('Store', 'Date', 'Norm_Fuel_Price'))
ggplot(data=sales.by.norm.fuel, aes(x=Norm_Fuel_Price, y=Norm_Sales, 
  group=round_any(Norm_Fuel_Price, 0.25))) + geom_boxplot()
sales.by.norm.fuel$Fuel_Price_Category <- ifelse(
  sales.by.norm.fuel$Norm_Fuel_Price < 0, 'Below Average', 'Above Average')
ggplot(data=sales.by.norm.fuel, aes(x=Norm_Sales)) + geom_histogram(binwidth=4) +
  facet_wrap(~Fuel_Price_Category)
above <- subset(sales.by.norm.fuel, Fuel_Price_Category == 'Above Average')
below <- subset(sales.by.norm.fuel, Fuel_Price_Category == 'Below Average')
var.test(above$Norm_Sales, below$Norm_Sales)
t.test(above$Norm_Sales, below$Norm_Sales, var.equal=FALSE)
wilcox.test(Norm_Sales ~ CPI_Category, data=sales.by.norm.cpi)
```

Economic Factor | F-test | t-test | Welsh t-test | Mann-Whitney-Wilcoxon test
----------------|--------|--------|--------------|---------------------------
Unemployment    | 0.44   | 0.30   | 0.30         | 0.06
CPI             | 1.02e-7 | NA    | 0.05         | 2.04e-6
