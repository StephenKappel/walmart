...
How does unemployment relate to sales? It's not clear what we would expect the relationship to be. In general, we would think more unemployment leads to less spending, but Walmart is known for low prices, so poor economic conditions might actually cause more people to shop at Walmart. And, Walmart sells many essential goods (food, clothing, etc.) whose demand may not be highly elastic.

```{r}
ggplot(data=sales.by.unemployment, aes(x=Unemployment, y=Norm_Sales, group=round_any(Unemployment, 1.0))) + 
  geom_boxplot()
```

This plot seems to indicate that there isn't significant correlation between unemployment rate and normalized sales.

TODO: Perhaps we should be looking at normalized unemployment vs. normalized sales
