...
=======
Kaggle provides the data in the form of three CSVs. We combined the three source CSVs together to aggregate weekly sales by store and department into a single data frame. This was implemented using the merge function. For better visualization and analysis, we categorized certain fields in to classes.
>>>>>>> Stashed changes
...
=======
In order to make good predictions about sales, we need to consider much more than just the economic factors we've considered so far. For example, the next couple of plots show that dates will also be important in our prediction; we observe seasonality and trending patterns in the timeseries.
>>>>>>> Stashed changes
...
store.level <- fillMarkDownZero(store.level)
store.level <- add.normalized.col(store.level, 'MarkDown1')
store.level <- add.normalized.col(store.level, 'MarkDown2')
store.level <- add.normalized.col(store.level, 'MarkDown3')
store.level <- add.normalized.col(store.level, 'MarkDown4')
store.level <- add.normalized.col(store.level, 'MarkDown5')
>>>>>>> Stashed changes
```
...
sales.formula <- Norm_Sales ~ Norm_Unemployment + Norm_CPI + Norm_Fuel_Price +
  ...
  Lag_4_Sales + Lag_51_Sales + Lag_52_Sales + Lag_1_53_Diff + Month +   Norm_MarkDown1 + Norm_MarkDown2 + Norm_MarkDown3 + Norm_MarkDown4 + Norm_MarkDown5
